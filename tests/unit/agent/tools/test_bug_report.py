...
from src.agent.tools.bug_report import (
    BugReportStatus,
...
class TestReportBug:
    ...
    async def test_successful_bug_report_submission(
        ...
        with (
            patch("src.agent.core._session_manager") as mock_session_mgr,
            patch("src.agent.tools.bug_report.Path") as mock_path_cls,
            patch("uuid.uuid4") as mock_uuid,
            patch("src.agent.tools.bug_report.datetime") as mock_datetime,
        ):
            # Setup mocks
    ...
    async def test_conversation_context_capture_last_10_messages(self, mock_run_context):
        ...
        with patch("src.agent.core._session_manager") as mock_session_mgr:
...
class TestWriteBugReportJsonl:
    ...
    def test_file_creation_on_first_write(self, tmp_path):
        ...
        # Arrange
        with patch("src.agent.tools.bug_report.Path") as mock_path_cls:
            file_path = tmp_path / "data" / "bug_reports.jsonl"
