...
class TestCacheHeaders:
    ...
    async def test_the_hero_art_is_a_hashed_immutable_asset(self, lifespan_client):
        # The Welcome surface's banner. Pinned here because it is the headline asset of the first
        # screen and nothing else would notice its absence — the SPA renders `<img alt="">`, so a
        # missing file is a silent hairline strip, not a failing test.
        #
        # It moved out of `ui/public/` and into `ui/src/assets/` so that Vite content-hashes it:
        # at the bundle root it was served `no-cache`, which means a 420 KB revalidation on every
        # open of the Welcome surface. Under `assets/` it is immutable for a year.
        async with lifespan_client(build_app()) as client:
            response = await client.get(_asset(".jpg"))
    ...
    def test_no_unhashed_hero_survives_at_the_bundle_root(self):
        # The other half: `emptyOutDir` removes the old root copy on rebuild, and a stale one
        # would be served `no-cache` and fetched by nothing — dead weight in the committed bundle.
        assert not (spa.STATIC_DIR / "hero.jpg").exists()
        assert not (_MIRRORED_STATIC / "hero.jpg").exists()
