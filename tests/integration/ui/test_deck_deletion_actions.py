...
from src.ui.actions.deck_actions import on_cancel_delete_deck, on_confirm_delete_deck
...
class TestDeckDeletionConfirmation:
    ...
    async def test_confirm_delete_deck_removes_deck(
        ...
        with patch("src.ui.app._session_factory", session_factory):
            with patch("src.ui.actions.deck_actions._session_manager", session_manager):
                with patch("src.ui.action_callbacks.remove_all_actions", new_callable=AsyncMock):
                    with patch("src.ui.app.cl.Message") as mock_message_class:
                        ...
                        with patch(
                            "src.ui.components.sidebar.update_deck_sidebar", new_callable=AsyncMock
                        ):
...
class TestBackwardCompatibility:
    ...
    async def test_delete_deck_tool_returns_confirmation_signal(
        ...
        from src.agent.tools.deck_tools import delete_deck
