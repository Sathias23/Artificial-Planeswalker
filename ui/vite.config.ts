    ...
    projects: [
      {
        ...
        test: {
          ...
          // The two suites that shell into a real tool (ESLint/stylelint, a Vite dev server) pay
          // a cold start measured at well over the 5,000 ms default, so they run under
          // `npm run test:gates` (`vitest.gates.config.ts`) with their own timeout. Excluding
          // them here is what lets this project keep vitest's default timeout, so a genuinely
          // hung test is still caught quickly. Spread over the defaults rather than replacing
          // them: a bare `exclude` would drop the node_modules/dist entries vitest ships.
          exclude: [
            ...configDefaults.exclude,
            'tests/lint-gates.test.ts',
            'tests/devProxyRoundTrip.test.ts',
          ],
        },
      },
      {
