/**
 * Where the reconnect loop is mounted, and what its three signals actually do.
 *
 ...
 *
 * 3b. **a `suggestions` push** → the agent view opens, with no click anywhere in
 *    it (UX-DR34). This is the FOURTH signal and the
 ...
 *    that kind — **the two kinds act differently here** — and the poll **only if it had
 *    stopped** (`restartPollIfStopped`), for BOTH kinds and both `deck_changed` branches alike.
 *    The poll asymmetry is deliberate: the deck may have changed, so the deck path always runs;
 *    but the poll is already working unless `RETRIES_QUIETLY` said its panel does not retry,
 *    and restarting a working poll would be a second polling mechanism racing the first.
 *
 * ================= WHAT A REFETCH IS, AND WHAT IT IS NOT (NFR-04) =======================
 *
 * *"Something changed, refetch."* No diffs, no patches. The `deck_changed` payload's `deck_id`
 * is read, TOTALLY (missing payload, missing key, `null` and a blank string all fold to `null`,
 ...
 *
 * The part `contracts.py:902-905` warns about: `active_deck_changed`'s
 * payload is never read, because *which deck am I looking at* has exactly one correct
 ...
 * delivered to the same handlers. There is *exactly one socket per tab*, and this is where that
 * is true or not.
 *
  ...
  useEffect(() => {
    const socket = createAgentSocket({
      ...
      onSystemEvent: (event) => {
        ...
        // edited" against "which deck you are looking at changed" — and they get different
        // verbs:
        //
        //   - `active_deck_changed` re-drives the boot, payload unread: the
        ...
        if (event.kind === 'deck_changed') {
          // TRIMMED, not merely blank-checked: the id is compared verbatim
      ...
      // THE PUSH, AND THE WHOLE OF WHAT THIS SEAM DOES WITH IT. Passed straight
      ...
      // THE SECOND PUSH KIND, in the sibling's exact shape and for its exact
      ...
      // THE THIRD PUSH KIND, same shape, same reasons.
      onTierList: openTierListPush,
      // THE FOURTH AND LAST PUSH KIND, same shape, same reasons — with it every
