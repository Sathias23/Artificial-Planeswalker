/**
 * The decklist, partitioned by board and grouped by card type — derived ONCE (FR-05).
 *
 * The epic's reason is verbatim and it is the whole point of this module existing rather than a
 * `useMemo` in a component: *"so the grid and the list panel cannot disagree"*. The card-art
 * grid, the deck-list panel and the mana curve all read the same derived
 ...
 * Measured against the live database: **0 of 1,999 live deck rows** are such a printing.
 * **Latent, not live** — declared here rather than closed by fetching 99 card records to fix a
 * case no deck contains.
 *
 ...
 * today**.
 *
 * ⚠️ **82 is not the whole disagreement.** {@link groupOf} and the whole-string policy disagree
 * on **116** corpus cards in
 ...
 * the front's. Measured across the corpus: **29 distinct type lines, 0 of them in
 ...
 *
 * ONE list serving both, because two lists are two things to drift. The order is the
 * conventional decklist order every Magic tool uses, so the group headers and the cold-open
 * *"the first card of the first type group"* both have something deterministic to rest on;
 * `deckGroups.test.ts` asserts the order itself, not merely the membership. (The "first card"
 * does not rest on this group order ALONE: within each group `boardsOf` sorts by
 ...
 *
 * **What first-match-wins decides, measured on live decks** — 88 rows carry more
 ...
 *
 * `Other` is the residual, and it is what makes conservation possible: a card the
 ...
 *
 * ⚠️ A trap the next re-measurement will walk into: there ARE two live rows named "Pym
 * Particles", but they are two different PRINTINGS in two different decks: `msh` #70 is an
 * ordinary `Sorcery` and groups as one. Counting by NAME finds two; counting by GROUP finds one,
 * and this bucket is about the group.
 */
 ...
 * this repo — the spec says `' // '`, the Python viewer splits on `'//'` and trims — and
 ...
 *
 * Args:
 *   typeLine: The card's `type_line`, verbatim. Measured: **0 of 38,261 corpus rows**
...
/**
 * Which group one type line belongs to.
 *
...
/**
 * A deck's cards, partitioned three ways and grouped by type inside the mainboard.
 *
 ...
 * here means the grid, the list and the curve inherit ONE partition. Measured cost of not doing
 * it: 41 sideboard rows and 16 commander rows would join the type groups and inflate every count
 * on screen.
 */
...
/**
 * Partition a deck's cards into boards and type groups.
 *
 ...
 * side of the non-sideboard split a row lands on. Measured: **0 of 1,999 live rows
...
/**
 * **THE ONE ANSWER TO "IS THIS DECK EMPTY?"**
 *
 * ================= WHY A FIFTH PREDICATE WOULD HAVE BEEN A DEFECT ======================
 *
 * Four predicates about "does this deck have anything in it" existed, and each answers a
 * DIFFERENT question:
 *
 ...
 *
 * ================= IT IS SIDEBOARD-INCLUSIVE, DELIBERATELY ==============================
 *
 * A deck whose only content is a sideboard HAS cards. Two reasons, and the second
 * is the one that matters here: the deck list renders a focusable row per sideboard card, so
 ...
 * **no artefact describes**. It is unreachable from live data (measured against the shipped
 * database: **0 of 42 decks** have zero mainboard rows and ≥1 sideboard row; 41 sideboard
 ...
 *
 * The UX-DR36 half of the residue, stated too: on that deck the LEFT
 * column carries no visible text at all — an empty `<ul>` in the untitled panel, both analysis
 * panels self-hidden on their own zero totals — which is the closest reachable state to a
 * blank viewport. The never-blank criterion holds only because the right column
 ...
 * curve total and a zero pip total — and those two guards are NOT this one. That distinction
 * is written down in `AnalysisRow.css`, and `App.test.tsx`'s land-only test is what stops the
 * two being conflated.
 *
