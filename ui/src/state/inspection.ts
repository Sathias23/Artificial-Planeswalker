/**
 * What the companion is currently showing in the card detail panel (FR-17, AD-12, UX-DR20,
 * UX-DR22, UX-DR39).
 *
 * ================= THE SPINE SENTENCE, NARROWED IN THE OPEN =============================
 *
 * `deck.ts` quotes the architecture spine verbatim: *"its state comes from exactly two inputs —
 * REST responses and WebSocket messages. Nothing else may write the store."* **A hovered or
 * pinned card is a third kind of input** — so the sentence is narrowed here, deliberately and in
 ...
 * that reason so the narrowing has one address rather than being re-derived elsewhere.
 *
 * The counterweight: **per-tile state must not be lifted** — 99 tiles sharing one loaded-set in
 * the store is a store write this slice is not allowed to make. That rule is about NINETY-NINE
 * values, one per tile, each private to the component that owns it. This is ONE value, read by
 * components in two different subtrees (the grid's tiles and the right column's panel) and by a
 * third, the agent view, that is not mounted inside either. A value two subtrees must agree on
 * is precisely what a store is for.
 *
 * ================= IT LIVES AT MODULE SCOPE, AND THAT IS FR-17's OWN REQUIREMENT =======
 *
 ...
 * the agent view is rendered in `AppShell`'s `overlay` slot — a different subtree that mounts
 * and unmounts. The behaviour is pinned end to end in `App.test.tsx` ("ESC CLOSES THE VIEW AND
 * THE PIN SET FROM A ROW SURVIVES").
 *
 * ================= THE API IS LOCATION-AGNOSTIC, ON PURPOSE ============================
 *
 * Nothing below is named for a tile. The deck rows, the flip control (which must touch NONE of
 * this — a flip is not an inspection) and the agent-view thumbnails are all consumers, and a
 * `setHoveredTile` would have made two of the three read as a misuse of something else's API.
 * The verbs are what happens, not where.
 *
 ...
 * interactive element. **The flip control sits INSIDE the tile's `<button>` and suppresses
 * them with `stopPropagation`** — which works because the tile's handlers are on the button
 * itself and `click` bubbles to it. That contract is stated in `CardTile.tsx` too.
 *
 * ================= ESC IS LAYERED, AND THE AGENT VIEW WINS =============================
 *
 * UX-DR39: *"Esc closes the topmost thing (an open agent view first, then an active pin)."*
 * `CardDetail` registers a document-level `keydown`, in the BUBBLE phase, that calls
 * {@link clearPin}. **The contract the agent view relies on: it registers its own `keydown` on
 ...
 *
 * `AgentView.tsx` holds the capture half, and `App.test.tsx` asserts one Esc closing a view
 * while a real pin survives it — over both live document listeners, in one app.
 *
 ...
 * ordinary states.
 *
 * **Why the two transients are TWO SLOTS and not one.** Hover and keyboard focus are
 ...
 * argument) and a tile asks {@link useIsLiveTarget} without knowing what a deck is. With the
...
export interface InspectionState {
   ...
   * {@link coldOpenTargetOf}; `null` for a deck with no cards.
   */
...
/**
 * Whether a card can be inspected at all (UX-DR22).
 *
 ...
 * The grid never renders the unknown-card variant: an image failure draws the NAMED
 ...
 * **the agent-view thumbnails**, whose ids do not come from a deck at all — and every one of
 * those reaches a target through the two verbs below. One refusal, at the one door.
 *
 * ==== WHAT IT DOES **NOT** REFUSE, WHICH IS THE LOAD-BEARING HALF =====================
 * `undefined` means *"never seen"* and nothing else, so an unseen id is inspectable
 ...
 * touching a wire token (no `switch (entry.reason)` anywhere).
 */
 ...
 * target (with one shared slot, a `mouseleave` could strand a still-focused tile).
 *
 * Args:
...
/**
 * Both transients die at once — the deck-transition clear (`CardDetail`'s boards effect): a
 * hover or a focus pointing into a deck that just left the glass is stale by
...
/** Release the pin: Esc, or the panel's unpin control. Idempotent. */
export const clearPin = (): void => {
  useInspectionStore.setState({ pinnedId: null })
}

/**
 * Whether a card is in a deck's LIST — all three boards, by `card_id`.
 *
 ...
 * row) and is "in the deck's list", so its DEPARTURE is the rule working — while
...
/**
 * The membership-transition eviction: at a boards replacement, release the pin ONLY if its card
 * was in the departing decklist AND is absent from the new one. Everything else survives — a
 * card in both lists (the same-deck refetch), a card in neither (a pinned suggestion: it falls
 ...
 * what a pin points at. The fall-back needs no code here:
...
/**
 * The cold-open target: the first card the GRID DRAWS (AD-12).
 *
 ...
 * RULE.** What keeps it from drifting is a test in `CardDetail.test.tsx` that renders `CardGrid`
 * over the same boards and asserts this function's answer is the id of the first tile the grid
 * actually produced. A repeated expression checked against its original is a different thing
 * from a second derivation (AD-12), and the check is what makes the difference real.
 *
 ...
 *   returns three empty boards and a deck with no cards is a real state.
 */
...
/**
 * Whether this card is the one under inspection — the tile's `live` state (DESIGN.md's
 ...
 * the deck rows call the identical hook without going through the grid at all.
 */
 ...
 * `aria-live` speaks when its content changes, and a pin announces *exactly once* —
