 ...
 *
 * AN `<h2>`, BY UX-DR44 TAKEN AS WRITTEN. The rule reads "panel titles and
...
export interface GroupHeaderProps {
   ...
   * `ReactNode`, unlike Panel's `title` — recorded, not accidental:
   * `title` is typed `string` because it doubles as the region's `aria-label`, which must
   ...
   * mounted to name a group, so an empty label is caller error — the deck list always has a
   * type name to give it.
   */
  label: ReactNode
  /** The group's size, right-aligned in the numeric role. `0` renders as "0". */
  count?: number
}
