...
/**
 * The mana curve, derived from the deck already in the store (story c4-8, FR-05, UX-DR17).
 *
 ...
 * and none is needed** — the first panel in the epic that is complete the instant the deck
 * lands. c4-6 needed the deck-wide sweep for `card_faces`; c4-7 needed it for
 * `card_faces[0].mana_cost`; this needs neither, so **c4-6's no-re-drive window is not
 * triggered here**. Stated so the next reader does not add a dependency this panel does not have.
 *
 * ================= "DFCS BUCKET BY THEIR FRONT FACE" IS ALREADY TRUE, FOR FREE ==========
 *
 * Measured at `0fdb41b` against the shipped database: of the **2,830** faced cards carrying a
 * blank top-level `mana_cost` — the population that cost c4-7 a whole module and a dependence on
 * the sweep — `cmc` **equals the front face's mana value in 2,830 of 2,830 cases (100%)**. The
 * AC clause that reads like the expensive one is satisfied by reading one field. This is the
 * exact inverse of c4-7, where the front-face cost was blank for 87.8% of faced cards.
 *
 ...
 *
 * **Commander + mainboard; the sideboard is excluded.** The sideboard half was already ruled in
 * shipped source — `deckGroups.ts:199`, *"the sideboard is not part of the deck the curve and
 * colour panels describe"*. The commander half is specified in **no artefact**, and it matters:
 ...
 * MCP tool cannot answer the same question differently — the epic's *"the grid and the list
 * panel cannot disagree"* clause, one layer out.
 *
 ...
 * {@link CurveBuckets} is coupled to it in the type, so a story that wanted an eighth bucket
 * would have to say so in the open (decide-once ruling 17).
 */
 ...
 * benchmark set) and was declined for this story with the divergence recorded in the ledger. The
 * observable consequence, stated rather than discovered: **`analyze_mana_curve` and this panel
 * report different non-land counts for the same deck.**
 *
 * ================= WHOLE WORD, NOT SUBSTRING — AND THAT IS A CORRECTION ================
 *
 * The story proposed `frontFace(typeLine).includes('Land')`. Measured, that is wrong for two
 * corpus cards: `Lander Rizzi` (`Legendary Artifact Creature — Lander Rogue`) and the `Lander`
 * token (`Token Artifact — Lander`) are not lands and a substring test drops both out of the
 * curve. `deckGroups.ts:166-168` had already written down why — *"a substring test would
 * additionally group anything containing `Landfall`-shaped text wrongly"* — so the proposal
 * carried a defect its own primary source rejects. The word test costs nothing today (0 live
 ...
 *      verbatim, loose separator and all. c4-7 measured that the loose pattern INVERTS for a
 *      *name* (`'SP//dr, Piloted by Peni'` truncates to `'SP'`) and shipped a literal split
 *      there; a **type line** is the case the loose pattern was written for.
...
/**
 * Which bucket a mana value falls in (AC 6, AC 11, Q1, Q2).
 *
 * ================= BOTH ENDS ARE DECISIONS, AND NEITHER IS IN AN ARTEFACT ==============
 *
 ...
 *
 * **The bottom, which is the one that changes what a user sees (Q1).** Buckets start at 1, so a
 ...
 * dropped card would be invisible in a way c4-7's deleted price column was not.
 *
 * **Ruled: `cmc <= 1` folds into bucket 1.** The reasons are asymmetric and worth stating. A
 ...
 *
 * The rounding falls out of the same ruling and is therefore **moot rather than lucky**:
...
export interface CurveBucket {
  ...
  /** The SUMMED QUANTITY in this bucket, never a row count (AC 10). */
...
export interface CurveBuckets {
  ...
  /** Every bucket summed — the number Q12's hide condition tests. */
...
/**
 * The curve for a deck (AC 6–12).
 *
 * ================= DERIVED IN RENDER, NOT CACHED AND NOT MEMOISED (Q9, AC 12) ==========
 *
 ...
 *
 *   1. **A memo is a cache**, which is the thing the AC rules out.
 ...
 *      gain.
 *   3. Computing it beside `boardsOf` at store-write time would be both the cache the AC forbids
...
export const curveOf = (boards: DeckBoards): CurveBuckets => {
  ...
  // The commander board and the mainboard's groups; the sideboard is simply not read (Q5).
  ...
  for (const entry of rows) {
    ...
    // `+ entry.quantity`, never `+ 1` (AC 10). A ×4 row is four cards, the same rule
