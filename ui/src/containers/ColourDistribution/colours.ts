...
/**
 * The deck's colour balance, as pip counts (story c4-9, FR-05, UX-DR18).
 *
 * Its own module rather than a helper inside `ColourDistribution.tsx`, for the settled reason:
 ...
 * `frontFaceCost.ts` and `curve.ts` are the six precedents; this is the seventh.
 *
 * ================= "PROPORTIONAL TO PIP COUNT" RESOLVES TWO WAYS (Q1) ==================
 *
 ...
 *
 * Measured read-only at `1ed2e83` against the shipped database, keyed on deck **id**:
 *
 *   | measurement                                   | result                                 |
 *   |-----------------------------------------------|----------------------------------------|
 ...
 *
 * **RULED: the front face.** Three reasons, in order of weight. (a) It is the reading every other
 * surface in this epic already uses — `deckGroups.ts` groups by front face, `curve.ts` buckets by
 * front face, `frontFaceCost.ts` draws the front face's pips in the deck row, and UX-DR17/FR-05
 * say "front face" in writing; a bar counting both halves would be **the only surface in the app
 * that does not**. (b) The panel's question is *"does my mana base match my spells"*, and the back
 * half of an Omen is a cost you pay separately or never. (c) The bar and the deck list then agree
 * BY CONSTRUCTION — the epic's *"the grid and the list panel cannot disagree"* clause, one layer
 * out. The cause is the current-Standard **TDM Omen dragon cycle**, sitting in four live decks.
 *
 ...
 *
 * ================= AND THAT MEANS THIS PANEL DEPENDS ON HYDRATION (Q2) =================
 *
 * `cmc` rode on `CardSummary`, which is why c4-8 was the first panel in four to escape the sweep.
 * A pip count does not: **2,830 of 3,225 faced cards (87.75%) carry a blank top-level
 ...
 * R 5 · W 4).
 *
 * So **this is the epic's first panel whose PERCENTAGES MOVE AFTER FIRST PAINT**, and two things
 * follow that are written down rather than discovered:
 *
 *   1. **c4-6's accepted no-re-drive window applies, cited rather than re-opened.** c4-2's
 *      edge-triggered recovery only re-boots from `refused`/`none`, so a backend blip *during*
 *      the sweep leaves those pips permanently missing with no error state. That is the
 *      documented posture (c4-6 review ruling 1), not a hole this panel papers over with a retry
 *      it does not own.
 ...
 *
 * ================= WHAT ONE SYMBOL CONTRIBUTES (Q4) ===================================
 *
 ...
 * for nobody there — see this module's Q16 note below.
 *
 ...
 *
 * ================= LANDS ARE EXCLUDED, AND THE POLICY IS THE THIRD ONE (Q4 e) ==========
 *
 ...
 * ================= WHICH BOARDS (Q6) =================================================
 *
 * **Commander + mainboard; the sideboard is excluded** — identical to c4-8's ruling, and the
 * sideboard half is already written in shipped source: `deckGroups.ts:197` says *"the sideboard
 * is not part of the deck **the curve and colour panels** describe"*, naming this panel by
 * function. `src/mcp_server/tools/deck_analysis.py:171-173` makes the same partition, so the
 ...
 *
 * **No numeric mana-value parser (Q12).** `curve.ts:36-40` re-homed one here by name, and this
 * story declines it: a pip counter walks {@link ManaColour} lists and **never adds a generic
 * cost**, so nothing in this file converts a cost string to a number. Writing one anyway — to fix
 * a `cmc` divergence with **0 live exposure** — is scope this panel cannot justify. The honest
 * home is a story that needs a mana value, and there is none in Phase 1; `curve.test.ts`'s pin
 * stays, with its comment corrected to name that condition rather than this story.
 *
 ...
 *
 * **Q16 — the Python is not changed, and the divergence is now OBSERVABLE.**
 * `compute_pip_signals` (`mana_base.py:343-390`) feeds `_mana_efficiency_score` and thence
 * `assess_deck_power`, whose calibration benchmark set is Epic 5's frozen artefact. It differs
 * from this file on **five axes at once**: hybrids (29 live copies / 10 decks), Phyrexian (7 /
 * 10), generic-hybrids (0 live), the sideboard (87 pips / 5 decks, included there) and split
 * cards (27 rows / 53 copies). Ledgered with a deck count beside each, because *"a bare number in
 * a ledger entry is not checkable when three tests all sound like the same policy"*.
 */
...
export interface ColourSegment {
  ...
  /**
   * The SUMMED QUANTITY of pips of this colour, never a row count (AC 12). A ×4 row of
   ...
   *
   * ⚠️ **THE DISPLAYED PERCENTAGES NEED NOT SUM TO 100** (Q14, AC 21), and that is the ruling
   * rather than an oversight. Three equal colours print `33% · 33% · 33%`. The alternative — a
   ...
   * so the browser divides and the segments always fill the bar exactly (AC 19).
   *
...
export interface ColourDistribution {
   ...
   * nothing at all rather than an empty bar (Q10, AC 32).
   */
  readonly total: number
}

/**
 * The colour distribution for a deck (AC 6–14).
 *
 * ================= DERIVED IN RENDER, NOT MEMOISED — AND THE PRICE IS DIFFERENT (Q5) ===
 *
 ...
 *
 * **But the honest difference from c4-8 is that this one runs more often.** c4-8's derivation ran
 * once per deck change; this one also runs once per **hydration**, because the panel subscribes to
 * the whole cache map (Q3) — ~99 renders on the 99-card deck during a cold-open sweep — and it
 * parses ~99 cost strings rather than reducing ~99 numbers.
 *
 ...
 * ~2.3× the extra tokenising predicts, and within a µs or two of c4-8's recorded 24 µs for the
 * same function. A single warm call reads ~70 µs. So the **whole cold-open sweep costs ~4.8 ms of
 ...
 *   cards: The hydration cache's whole map, from ONE `useCardStore` subscription (Q3). Passed as
...
export const coloursOf = (
  ...
  // The commander board and the mainboard's groups; the sideboard is simply not read (Q6).
  ...
  for (const entry of rows) {
    ...
    // The three shapes, already solved (Q2): split first, then a real top-level cost, then the
    ...
    for (const token of parseManaCost(frontFaceCost(entry.card, cards[entry.card_id]))) {
      ...
      // EVERY colour the symbol carries (Q4 a): one for a plain pip, two for a hybrid, and none
      // at all for generic and `{X}`. `+ entry.quantity`, never `+ 1` (AC 12).
