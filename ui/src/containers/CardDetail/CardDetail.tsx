 ...
 * makes reading a deck one continuous motion (FR-17, UX-DR20, UX-DR36, UX-DR44, UX-DR45).
 *
 * ================= WHAT HYDRATION ACTUALLY BUYS, MEASURED ==============================
 *
 ...
 * The other things that call buys are not decorative — it warms the cache the flip control flips
 * through, it is what gives the cache's terminal-after-three asymmetry something to be visible
 * on, and the sweep criterion (*"the cursor sweeps the whole grid twice → each distinct card is
 * fetched at most once"*) is satisfied by `hydrateCard`'s in-flight dedupe and attempt cap
 * rather than by anything written here.
 *
 * ================= NO SPINNER, AND IT IS STRUCTURAL (UX-DR36) ==========================
 *
 ...
 * CARRIES whatever was known before, so nothing already on screen goes blank. The only
 ...
 * EXTENDED rather than duplicated (`face` sits beside `size` in the same builder), and no
 * Scryfall host appears anywhere: art goes through this app's origin, always
 * (AD-11, `tests/no-scryfall-hosts.test.ts`).
 *
 * ================= WHAT IT IS NOT: A MODAL, AND NOT A LIVE REGION ======================
 *
 * **Not a modal** (UX-DR38 — it *"neither stacks nor traps"*): no focus trap, no return-focus
 * contract, no `aria-modal`. It is a column that is always there.
 *
 * **Not a live region** (UX-DR44/45), and that is a fixed defect rather than a preference: with
 * `aria-live` on the panel, sweeping a cursor across a 60-card grid would fire one polite
 * announcement per card and flood the queue. So transient changes announce NOTHING, and only a
 * pin speaks — once, through the separate region at the bottom of this file.
 * `CardDetail.test.tsx` asserts the ABSENCE of `aria-live` on the panel, so it cannot be
 * reintroduced quietly.
 *
 * ================= ESC IS LAYERED, AND THE AGENT VIEW MUST WIN =========================
 *
 ...
 * pin. **The contract the agent view relies on: it registers its own `keydown` on `document` in
 * the CAPTURE phase, closes itself, and calls `stopPropagation()`.** Capture at the document
 ...
 *
 * **The layering is tested in two places.** `AgentView.test.tsx` proves the phase mechanism
 * against a stand-in bubble listener, and `App.test.tsx` proves the real thing — one Esc, a real
 * pin set by a real click, both document listeners live, the view closing and the pin surviving.
 *
 * ================= WHAT THIS PANEL DELIBERATELY DOES NOT DO ============================
 *
 * **No price, and that is satisfied BY ABSENCE.** `DESIGN.md` puts a price beside the type
 * line and `EXPERIENCE.md` says *"prices render only when present in local data"* — and there is
 * no price field on the wire (`types.d.ts:325`: *"There is no price data of any kind in this
 * record."*). The endpoint ships no price field at all rather than a permanently-null one, so
 * there is nothing to render conditionally: no zero, no placeholder, no empty slot, and no
 * `price` in `Card` for a future edit to reach for. `CardDetail.test.tsx` asserts that at the
 * TYPE, which is the only place an absence can be asserted.
 *
 * **No deck list, no format check** — those are their own components, stacked beneath this
 * panel in the same column. **No skip link** — it lives outside every landmark; this panel only
 * exposes the handle it targets (see the frame's `id` below).
 *
 * ================= THE TAB STOPS THIS PANEL ADDS (UX-DR40) =============================
 *
 * **The unpin control is a Tab stop UX-DR40's enumerated order does not contain** — UX-DR20
 * requires the control and specifies nothing about it. It sits in the panel header between the
 * title and the panel body, exists only while a pin is active, and activating it moves focus to
 * the panel's `<h2>` (the same element the skip link targets — one focus home, not two).
 * **The oracle scroller is the other**: content inside `overflow-y: auto` must be reachable
 * from a keyboard, so it carries `tabindex="0"` — see its own comment below, and the scoped
 * `[tabindex]` assertion in `CardDetail.test.tsx` that permits exactly that one.
 */
 ...
 * `ManaCost` all use — the rule that emptiness is never truthiness. It is DUPLICATED VERBATIM
 * rather than imported, and that is recorded rather than silently repeated: if the copies keep
 * multiplying, the honest move is a shared module beside `src/components/filled.ts`, whose
 * header states the rule for exactly this case (*"a helper shared by two components does not
...
/**
 * A hydrated record, read at **the face the reader chose**.
 *
 ...
 *
 * **The per-field `??` is deliberate, and a per-record choice would be wrong**; there is a
 ...
 *
 * **`card_faces` is indexed DIRECTLY, and the premise is stated.** `resolve_face_images`
 ...
 * Whoever meets the first one owns that case. An out-of-range index falls through to the
 * top-level fields rather than throwing, which is the same totality every other read here has.
 *
 * **This is the one place the panel calls a card by a different name than the tile does**, and it
 * is deliberate rather than an oversight of the rule that *"four surfaces must not call one card
 * by two names"*. That rule is about surfaces showing the SAME thing; this panel is showing one
 * FACE — the very face whose picture is above the text — and `EXPERIENCE.md` says so directly:
 * *"while a DFC tile is showing its back face, hovering or pinning it targets that face: the
 * detail panel shows the back, its name, and its oracle text."*
 */
 ...
 *
 * Every non-hydrated arm reads `entry.summary`, which is what makes *"does not blank anything it
 * was already showing"* structural: `hydrateCard` carries the summary INTO the
 ...
 * field exists to prevent. `panelFor()` is not called on this path and
...
export function CardDetail({ boards }: CardDetailProps) {
  ...
  // WHICH FACE THE READER CHOSE. Read from the same module-scope slice the
  ...
  //
  // TWO CALLS, ONE PER FACE, exactly as the tile makes them — and at `size=large`, so these are
  // four distinct browser-cache keys per flippable card across the two surfaces. The back render
  // is only mounted for a flippable card, so an ordinary card still costs exactly one
  // `?size=large` request.
  //
  ...
  // THE ANNOUNCEMENT IS CAPTURED DURING RENDER, NOT IN AN EFFECT (UX-DR45).
  //
  // Not `useEffect(..., [pinnedId])` — ESLint's `react-hooks/set-state-in-effect` rejects that,
  // and rightly: a `setState` in an effect is a
  ...
  //
  // WITH ONE LATE DOOR: a pin of an id whose cache entry carries no name yet — a population the
  // agent view's thumbnails create, and the slice deliberately does not refuse — would capture
  // `''` and stay silent forever, turning "exactly once" into zero. So a
  ...
  //
  // A DECLARED DIVERGENCE, NOT AN OVERSIGHT: for the 6%-of-a-deck faced population the name
  // captured here is the summary tier's COMBINED one ("Clearwater Pathway // Murkwater Pathway")
  // while the panel renders the front FACE's once hydration lands — the reader hears one name
  // and reads another. Deliberate, because re-announcing on hydration is the exact flood the
  // not-a-live-region rule closed; how it SOUNDS is the manual-testing checklist's.
  ...
  // THE COLD-OPEN TARGET, WRITTEN ONCE PER DECK. In an effect rather than during render because
  // it writes a store, and `boards` is the dependency because it is the deck's identity as far
  // as this panel is concerned — `deck.ts` derives it once at write time, so it changes exactly
  // when the deck does.
  //
  // AND THE DECK TRANSITION IS WHERE AN INSPECTION DIES. The slice survives this panel
  // unmounting on purpose, which means nothing in it can know a
  ...
  //
  // THE PIN'S CLEAR IS CONDITIONAL: eviction is a membership transition — the pinned card was
  // in the DEPARTING decklist and is absent from the new one — decided by `evictDepartedPin`
  // over the two lists and nothing else, which is what lets a pin survive the same-deck refetch
  // every agent-edit settle produces (a reference comparison would release it every time) and
  // lets a pinned SUGGESTION survive every refetch with no special-casing. The TRANSIENTS still
  // die on every replacement: hover and focus are ephemeral by contract and stale by
  // construction whichever deck arrives. This runs at the boards-effect seam and NOT in `deck.ts`,
  ...
  // ESC RELEASES THE PIN (UX-DR39). On `document`, because the pin can be set from a tile in the
  // other column and released from anywhere — a listener on this panel would only work while
  // focus happened to be inside it. See the header for the layering contract the agent view
  // relies on, and for where it is tested.
  useEffect(() => {
    const release = (event: KeyboardEvent) => {
      // `isComposing`: an Esc that cancels an IME composition is not a request to release a pin
      // — reachable because the agent view puts a text input on the page. `defaultPrevented`: an
      // Esc some other handler has already consumed is not this listener's to act on either.
  ...
  // of the page. Focus moves to the panel's `<h2>` — the one element the skip link targets too,
  // so the two converge on a single focus home. `tabIndex = -1` is set IMPERATIVELY and removed
  // on blur: the panel at rest carries no `[tabindex]` outside the oracle scroller, which is
  // what the not-a-modal assertion checks.
  //
  // ==== THE HAND-OFF ITSELF IS `../focusHome` ==========================================
  // The skip link moves focus to THIS heading, so the two callers need the identical hand-off,
  // and "exactly one focus home and one implementation of it" is the requirement.
  // `CardDetail.test.tsx` asserts the behaviour from this side.
  //
  ...
  return (
    ...
    /* `id` — THE SKIP LINK'S TARGET HANDLE. The only AUTHORED DOM id in `ui/src` (the agent
       view mints its heading's from `useId()`, which is the distinction `focusHome.ts` draws: an
       id two modules in different directories must agree on is
    ...
    >
      <Panel
        ...
        level="overlay"
        badges={
          /* THE UNPIN CONTROL, in `Panel`'s existing badges slot and rendered ONLY
        ...
        <div className="card-detail-content">
          {targetId === null ? null : isUnknownCard(entry) ? (
            /* The app does not know what this card is. Reached only if a card BECAME unknown
               ...
               first place — so this is the honest render for a state that should not
               occur rather than a path with a population. */
            <CardPlaceholder variant="unknown-card" cardId={targetId} />
          ) : (
            /* ONE ART BOX FOR BOTH OUTCOMES, not one per branch. The flip control has to sit
               inside it either way (a face that failed can still be flipped OUT of), and two
               boxes would be two places to mount it and two
            ...
            <div className="card-shape card-detail-art">
              {art === 'failed' ? (
                /* AD-11: a designed stand-in, never a broken-image glyph. The SAME 79-card
                   ...
                   the browser's own glyph. ON THE SHOWN FACE: `?face=1` is a different
                ...
                <>
                  ...
                  {/* THE SAME TWO STACKED FACES THE TILE DRAWS, from the same shared classes in
                      FlipControl.css. Sharing them rather than writing a second
                  ...
                  >
                    <img
                      ...
                      /* `alt=""`, DIVERGING FROM UX-DR48'S LITERAL WORDS AND FOLLOWING ITS LOGIC.
                         That rule keeps `alt={name}` on the detail panel "because there the
                         image is the only carrier" — measurably false for this component,
                         exactly as for the tile: the card's name renders in `--type-heading`
                         immediately beneath the art, inside the same panel. The name is
                         announced ONCE. For a flipped face the heading FOLLOWS the face, so the
                         name a reader hears and the face they see agree — which is the half the
                         tile's caption cannot manage. A manual-testing check, for a real screen
                         reader. */
                      alt=""
                      decoding="async"
                      onLoad={onFrontLoad}
              ...
              {/* THE PANEL'S OWN COPY OF THE CONTROL (UX-DR15) — "pinned to its art's
                  ...
                  than the `<button>`-in-`<button>` the tile has to route around. */}
          ...
          {/* DESIGN.md:387's order and roles: name in `--type-heading` with the mana cost
              ...
              panel's ONE heading is its `<h2>` title, which is what the skip link moves
          ...
          {/* THE ORACLE SCROLLER IS A TAB STOP (WCAG 2.1.1).
              Three things go together — the scoped `[tabindex]` assertion in the test, the stop
              enumerated in the header, and the scroller being focusable — and this is the third.
              The clamp is `max-height: 21em` with `overflow-y: auto`, and content that can
              ...
              Sleeper Agent`, 530 chars, in `Atraxa Counter Cabinet`, measured (103 of 38,261
              corpus cards can overflow; the clamp has never been OBSERVED to fire on a real
              ...
              the flip control avoids by never materialising mid-Tab. */}
      ...
      {/* THE PIN ANNOUNCEMENT, IN A REGION OF ITS OWN (UX-DR45).
          OUTSIDE the panel, which is the whole of the fix: the panel is not a live region
      ...
      {/* `visually-hidden` FIRST, then this element's own class — the shared clip-rect in
          `src/styles/` plus the one declaration only this consumer wants,
          `pointer-events: none`. Order is documentation, not cascade: the two rules touch
          no common property. */}
