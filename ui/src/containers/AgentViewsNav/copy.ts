/**
 * The agent-views nav's words.
 *
 * ================= WHAT IS HERE, AND WHAT IS DELIBERATELY NOT =========================
 *
 * Five strings, and the KIND pill labels are NOT among them. `Suggestions` / `Swaps` /
 * `Tier list` / `Card groups` live in `src/state/agentView.ts`'s `AGENT_VIEW_LABELS`, because
 * the word for a KIND has one owner and the state layer is it — a view's fallback title and its
 * pill label are the same word, and the state layer needs it before any container exists. A
 * copy module here holding a second copy of those four strings is precisely the drift that
 * boundary prevents. What is left over is what belongs to
 ...
 * copy.test.ts` gates it against that row. The
 ...
 *
 * {@link NAV_GROUP_LABEL} and {@link UNREAD_WORD} are AUTHORED here and also written into
 * `EXPERIENCE.md`'s nav-pill row, so the gate has an artefact to read for them too.
 *
...
/**
 * The group's visible label, left of the pills (the mock's kicker at
 * `Planeswalker Companion.dc.html:33-51`).
 *
 * **It exists because the group has no landmark.** UX-DR44 assigns the header nav none, and it
 * stays that way — the pills open overlays, they do not navigate, so a `<nav>` would be a
...
/**
 * What a pill says when its kind has never pushed this session (UX-DR28, UX-DR33's ninth
 * state) — transcribed from `EXPERIENCE.md:73`.
 *
 * **It reaches assistive technology two ways, and that is deliberate rather than belt-and-
 ...
 * exactly the hover-only disclosure UX-DR39 bans. So it ships as BOTH: a `title` for the
 ...
 * UX-DR28 names this dual mechanism.
 *
...
/**
 * The fifth pill's visible label (FR-18).
 *
...
/**
 * What the History pill says before the first push of ANY kind this session —
 ...
 * a `title` for the pointer AND a visually-hidden `aria-describedby` target for the
 * accessibility tree, both carrying this one string.
 *
