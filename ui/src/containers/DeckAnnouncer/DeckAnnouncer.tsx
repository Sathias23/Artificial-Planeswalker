...
/**
 * The deck-refetch announcement region — UX-DR45's deck-refetch channel: the app's
 ...
 *
 * A coalesced refetch completes silently for a screen-reader user: the card appears, its
 ...
 *
 * ================= AND IT IS SILENT BEHIND A MODAL =====================================
 *
 * The modal gate reads {@link useAgentViewIsOpen} as a fifth PRIMITIVE subscription — one
 * boolean, not `useOpenAgentView`'s content object, which would resubscribe this component to
 * every push's items and break the narrowing discipline above.
 *
 ...
 * a dialog about something else — and it would put text into a region the App tests pin empty
 * behind a modal (the drop/resume walk; the two App live-region CENSUSES pin shapes at rest and
 * mid-flight, not this). The next real refetch is the next real
 * announcement.
 *
 ...
 *
 * No format-check announcement: it would be a SECOND per-refetch announcement, and no artefact
 * gives it copy. No timers and no debounce — the refetch supersession IS the coalescing, pinned
 * at the store level as one settle per burst, which arrives here as one counter tick per burst.
 */
export function DeckAnnouncer() {
  ...
  // THE MODAL GATE'S ONE INPUT. A boolean off the agent-view slice — the fifth primitive
  ...
  if (announced.seen !== settles) {
    // `&& !viewOpen` is the modal gate's WHOLE mechanism, and it is one term in one expression
    // on purpose:
  ...
  // in-flight window (the `updating` marker) never leaks a word in here — App.test.tsx's
