...
| **Whether a deck-construction _rule_ was reimplemented in the backend shell.** c3-3's guard bans four families from `src/companion` — reading `.legalities`, importing anything from the validator module outside its projection surface, comparing the limits `60`/`15`/`4`, and rebuilding a set of Scryfall format names. **`1` is deliberately outside the comparison family**: the singleton limit is 1, but `> 1` / `== 1` are ubiquitous in ordinary code, so banning it would be noise. A shell reimplementing the _singleton_ rule specifically would pass. Also invisible: a rule written in TypeScript, which no Python-side guard can see at all. | `tests/unit/companion/test_routes_format_check.py`, the `_LIMIT_LITERALS` docstring and `find_rule_violations`                                                                    | Review, on any story that computes legality client-side   |
