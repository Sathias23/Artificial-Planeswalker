...
| **Companion app** *(in development)* | `companion_set_active_deck` — puts a saved deck on the companion's live browser view; reports `app_not_running` when the companion isn't up |
...
Install the plugin from this repo's built-in marketplace to get all 20 tools **and** the four
...
you the 20 tools *and* the four deckbuilding skills. If Codex also auto-surfaces this repo's
...
`build_search_index` to index them). Desktop loads the 20 tools; the four skills are a Claude Code
plugin feature.
</details>
