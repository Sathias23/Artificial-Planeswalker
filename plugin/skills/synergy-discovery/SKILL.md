...
- **`companion_show_groups`** — the engines, packages, and combo clusters you surface, each as a
  titled group with its rationale paragraph (groups may include cards not yet in the deck).
- **`companion_show_suggestions`** — a flat list of synergy candidates with one-line reasons.
