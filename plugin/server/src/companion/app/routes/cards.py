...
``decks.py``'s docstring is written entirely about deck reads: they share the uuid constraint,
the repository call and the row, so splitting them would publish two names for one thing. Adding
a route to an **existing** router owes nothing to ``build_app()``'s ordering block and nothing to
``test_spa.py``'s differential router list.
...
**This module holds a lookup and a proxy, and no projection.** ``CardRepository.get_by_id``
already returns the exact ``Card`` schema **both** routes need, so AD-1's "no second card shape in
this shell" is satisfied by writing no projection at all — no ``from_deck``-style constructor, no
counts, no ``select``, and no image-specific query. The cost of that, stated rather than
optimised: an image request that has to *fetch* reads the whole card row. A warm disk-cache hit
does not — see ``read_card_image``.
...
"""The canonical Scryfall printing uuid, and nothing else.
...
bug. Unlike a deck id — which has no declared shape, so every unknown deck id is simply *not
found* — a card id has a declared shape, so "malformed" is decidable and gets its own answer.
...
end-of-input and that spelling is correctly refused (measured both ways). Anything
    ...
    Args:
        card_id: The Scryfall printing uuid — the value in ``cards.id``, and the same value a
            ...
            **The mechanism behind the Warning above, measured.** FastAPI solves dependencies and
            ...
            orders are pinned in ``test_routes_cards.py``. The consumer-facing half sits in the
            wire-visible ``Warning:`` section because this ``Args:`` section is truncated off the
            wire.
...
``application/json`` for the success case and the generated TypeScript lies), and the
...
def _image_response(body: bytes, content_type: str) -> Response:
    ...
    **One constructor, so cold and warm cannot drift.** A cache hit is the same
    ...
    else — the URL suffix is not consulted at all, not even as a fallback, because a header/suffix
    disagreement would be free to flip the whole media type between cold and warm for an
    accepted-but-unmapped type like ``image/webp``. No measured Scryfall response actually sends a
    parameter, and no browser behaves differently for one.
...
async def read_card_image(
    ...
    Args:
        scryfall_id: The Scryfall printing uuid — the same identifier and the same constraint as
            ...
            because the specification names this path ``/api/card-image/{scryfall_id}``. Anything
            outside
    ...
    Raises:
        CompanionError: ``card_not_found`` when no card carries the id; ``no_image_data`` when the
            ...
            opened** — the same token and the same response, deliberately, so a client cannot
    ...
    #
    # It is also read BEFORE the pacer (NFR-06). A check inside `pacer.slot()` would cache
    # correctly and pace a warm deck anyway: 99 tiles that never leave the machine would take
    # 9.9 seconds and would hold the CDN's rate budget against requests that issue no request at
    # all. `test_routes_card_image.py` measures that on the injected clock — a warm burst
    ...
    # two flavours of permanent to be, and the client draws the same placeholder either way. There
    # is deliberately no separate "no such face" token.
    ...
    # half of that placement is a decision:
    #
    ...
    # * OUTSIDE `pacer.slot()`, for the same reason the disk read is: a request that
    ...
    if remembered is not None and remembered.is_backing_off(scryfall_id, size, face):
        # DEBUG, not INFO, and the asymmetry with the record site below is the point: a hit costs
        # nothing and is client-driven, so at INFO a polling SPA repainting a 99-tile deck during
        # an outage would write ~99 lines per paint, every paint, unbounded. The bounded,
        # WORTH-a-line event is the window opening or escalating, logged once at the record site,
        # which a real (paced) fetch failure gates.
        logger.debug(
    ...
    # Every outbound byte in this application goes through this one call, and through the pacer
    # it is handed — which is what makes AD-11's "exactly one choke point" mechanically true
    # rather than aspirational: there is one call site and it cannot fetch without pacing. The
    # request may queue here; a cold 100-card deck is ~99 tiles at 100 ms apart, so the last one
    # starts ~9.8 s in. That is an EXPECTED observation (NFR-05 excludes first-fetch image paint),
    # it is deliberately not published to the wire, and it is written down for the tile author in
    # ui/README.md's blind-spot section.
    #
    # **The first and only `try`/`except` in this module**, deliberately — a SECOND one here would
    # be a smell. It is unavoidable under any design: the negative cache is keyed on id + size +
    # face, which `fetch_image` is not given and must not be (widening the one function whose
    # narrow signature is why no caller can fetch unpaced).
    #
    # `except CompanionError`, never `except Exception`. Three reasons, in order of weight:
    #
    ...
    try:
        ...
        if remembered is not None:
            # One INFO line per RECORD, never per remembered hit: a record is gated by an actual
            # paced fetch failing, so its rate is bounded by the fetch
    ...
    # between them. This is CM-2's observation seam: the unit tests prove once-per-key on
    ...
    # Cleared on success, which is the half of recovery a functional test misses: ending the
    ...
    # Written BEFORE the response is returned, not after. A write scheduled to happen "later"
    # caches correctly under every functional test — a mocked fetch and a mocked disk are both
    # instantaneous — and in production races the next request for the same tile. That is why the
    # cache is filled on the same await chain that serves the bytes.
    #
