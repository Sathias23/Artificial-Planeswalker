"""``GET /api/decks`` and ``GET /api/deck/{deck_id}`` — reading saved decks (FR-02).

The companion's first data routes, and a deliberately thin pair. Everything they need
already exists and is already gated: the session and both ``503`` paths arrive with
:data:`~src.companion.app.deps.DbSession`, the ``404`` token and its status live in
:mod:`src.companion.app.errors`, and the response shapes are ``src.data``'s own deck
schemas projected by their own constructors (AD-1 — this shell defines no second deck
shape and holds no SQL). What is left is two repository calls.

Both bodies are unwrapped (AD-16): the list is a bare JSON array and the detail is the
deck itself, with no ``status``/``count``/``decks`` envelope around either.
"""
