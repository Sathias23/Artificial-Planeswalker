"""The one canonical Spellbook combo record shape (AD-11).

``ComboRecord`` is the single frozen shape used verbatim by the combo-snapshot
repository, the pure scoring core's matcher
(:mod:`src.logic.assessment.combos`), and the edge's ``flags.combos`` serialization —
one vocabulary, no per-layer forks. It lives HERE, at the schema layer, rather than in
the core: the repository must return Pydantic schemas, never ORM (AD-5), and the strict
...
line of defense behind the importer's wire normalization. If Spellbook's
...
``ComboRecord`` does: the data-layer importer and the pure matcher
(:mod:`src.logic.assessment.combos`) must share ONE normalization — the DFC front-face
hazard has bitten more than once — and the strict ``data → logic`` import direction
...
def name_keys(name: str) -> tuple[str, ...]:
    ...
    The normalization policy: comparison is lowercased, and a multi-face
...
#: (AD-11). The importer normalizes wire casing into these; anything else is an error,
...
class ComboSnapshotMeta(BaseModel):
    ...
    through verbatim to the edge's ``data_vintage`` serialization — no datetime parsing,
