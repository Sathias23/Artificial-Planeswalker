"""FR5/FR8 mana-base & curve signals for deck-power assessment.

Raw numeric mana signals only: the curve distribution, average mana value, Karsten
land-count recommendations, and per-color pip/source consistency. Downstream modules map
these onto scores — the signal→0–100 ``mana_efficiency`` mapping is ``dimensions``', and
the hypergeometric consistency math over the land count is ``consistency``'s. Everything here is
...
Deliberately independent of :mod:`src.logic.mana_curve` (the legacy ``analyze_mana_curve``
...
serving its own tool; do not import it here.

Sideboard rows are NOT filtered — deck-composition policy belongs to the caller: filter
``sideboard=False`` first if you want played-cards-only signals (the classifier precedent).
"""
...
# ---------------------------------------------------------------------------
# Karsten constants — published coefficients are exact; tolerances are provisional.
# ---------------------------------------------------------------------------

#: Karsten's "cheap" cutoff for the card-draw/ramp land-count reduction: MV <= 2
#: (addendum §C / docs/deck-assess.md:125). The classifier deliberately does not pre-filter
#: by cost — this module owns the cutoff.
...
#: PROVISIONAL v1 value; tests reference this constant rather than hard-coding it.
KARSTEN_TOLERANCE_LANDS: Final = 2.0

#: The formula selector type — format choice enters as an explicit parameter so
#: this module stays profile-independent (the caller picks per format).
...
# ---------------------------------------------------------------------------
# Land detection — one helper owns the policy
# ---------------------------------------------------------------------------
...
def _is_land(type_line: str) -> bool:
    ...
    reading the classifier uses for ramp exclusion.
    """
...
def _cheap_draw_ramp_count(deck_cards: Sequence[DeckCard]) -> int:
    ...
    Joins the classifier tags back to ``cmc`` (the intended ``classify_card`` join):
    ...
    still one spell slot. Land-typed cards are never spell slots.
    """
    cheap = 0
    for deck_card in deck_cards:
        card = deck_card.card
...
def karsten_land_delta(
    ...
    Args:
        ...
        formula: Which published regression to apply — ``"commander"`` (99-card) or
            ...
            profile-independent; the caller selects per format.
...
# ---------------------------------------------------------------------------
# FR8 pip demand & colored sources
# ---------------------------------------------------------------------------
...
# requirement (documented v1 policy; tuning may weight them). Split cards store a
...
#      without basic types). Deliberately NOT reminder-stripped, unlike the
...
# Accepted v1 undercounts (calibration options): fetchlands (produce nothing
...
#: 3+-pip anchor of 20 is a PROVISIONAL extrapolation.
...
#: linear scale is an honest v1 that a benchmark pass can replace.
...
def _pip_cost(card: Card) -> str:
    ...
    # Attribute access because `card_faces` is typed as `CardFace`; `or ""` matters because a
    # named field Scryfall sent as null reads None just as a missing key did.
...
def _source_text(card: Card) -> str:
    ...
    Mirrors the classifier's empty-top-level fallback but deliberately without reminder-stripping
...
def compute_pip_signals(
    ...
    Args:
        ...
        formula: Selects the source-anchor table — ``"sixty_card"`` uses the published
            ...
            exactly like :func:`karsten_land_delta`'s selector.
