...
| 2 | CAP-2, CAP-3 | Greptile run 2 | `ui/src/state/connection.ts` (re-drive on every `live`, or subscribe before snapshot), `ui/src/state/deck.ts` (restart the stopped poll on a transient refusal), `ui/src/state/socket.ts` only if the reconnect signal changes shape, `ui/src/state/poller.ts` (a restart resets the emit dedupe identity so the probe's answer is written), `ui/src/state/poller.test.ts`, `ui/src/state/connection.test.ts`, `ui/src/state/deck.test.ts`, `ui/src/App.test.tsx` ("boots exactly once" may count differently; the no-boot-on-re-render assertion stays); doc ripple sites `ui/README.md` (boot triggers), `ui/src/state/formatCheck.ts` (priced re-ask), `ui/src/state/systemState.ts` (restart docstrings), `CHANGELOG.md` | `src/companion/app/static/`, `plugin/` |
