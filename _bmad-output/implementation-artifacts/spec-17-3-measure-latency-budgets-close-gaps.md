...
### 2026-08-22 — Greptile round (PR #98)

- patch: 1: (high 0, medium 1, low 0)
- addressed_findings:
  - `[medium]` `[patch]` Greptile r1 (the one finding, confidence 4/5): `_report_push` handed out a passing drain verdict on one or two valid runs — the story's 3-valid-run instrument-failure floor lived only in the workflow prose, and two of the review-pass unit tests even pinned the permissive behavior. Fixed both ends: `DRAIN_MIN_VALID_RUNS = 3` beside the budget constant; the reporter still prints the honest per-run figures but refuses the verdict (named stderr line, exit 1) below the floor; `cmd_push` refuses `--arm drain --runs < 3` upfront (each run is ~99 real CDN fetches — don't burn them on a set that cannot reach the floor). The two permissive tests corrected, plus `TestTheDrainEvidenceFloor` (5 tests: floor met → verdict; 2/3 valid → refusal naming the floor with figures still printed; over-budget at the floor still exit 2, the floor never softens the verdict; warm arm ungated; upfront refusal). Probe proof: planted floor removal → 3 RED by name through the full suite (3292 collected, exit 1); restored → 3292/0 green. The committed drain measurement (3/3 valid) is unaffected.
