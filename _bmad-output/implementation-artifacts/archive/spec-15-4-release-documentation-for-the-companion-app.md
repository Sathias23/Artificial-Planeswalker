...
MERGED via PR #88 into `feat/companion-epic-15` at `71a2ccb` (2026-08-19). Five commits: `085c9c0` the docs, `8447c26` the prose guard, `d1cdaa0` the 30-item review patch set, `5125660` the closeout, `79452cd` the Greptile P2. All five CI checks green. THE HEADLINE: the story was first COMPLETE WITH ITS OWN SUBJECT MATTER GATED BY NOTHING — 148 README lines quoting exact runtime strings with no test tying them to source. The build workflow's Matrix Test Audit caught it. `test_companion_docs.py` now carries 21 assertions and ZERO hand-typed README strings: announcements come out of `server.py`'s AST (f-string slots rendered as wildcards, then checked against HOST/DEFAULT_PORT), exit statuses out of `__main__.py`'s, panel copy out of `copy.ts` keyed on the KEBAB SPELLING of the shipped `database_not_initialized` token. Re-verified independently: DEFAULT_PORT 8765->8766 with the README UNTOUCHED reddens 3 doc tests. 17 firing proofs, all through `probe_harness`, tree staged before every plant. REVIEW was three layers, 34 findings: 0 intent gaps, 0 bad-spec loopbacks, 30 patched, 3 deferred, 3 rejected — and FOUR of the findings were proven by planting, ALL FOUR having passed the first guard cut green (a stale `port 9999` fallback, a dead anchor, a Requirements bullet claiming 'Node 22 or newer is required' against the guarded section's opposite, and a full revert of the imagery attribution). Two of the review's OWN new assertions then passed their first firing proof, were caught as vacuous, and were rescoped before landing. GREPTILE EARNED ITS KEEP ON A DOCS-ONLY PR (1 P2, valid): 'the partial database cannot be deleted or replaced' is TRUE ON WINDOWS ONLY — on POSIX the unlink succeeds while pooled connections keep the old inode, so a re-import writes a file the app never reads and the page never heals, silently contradicting FR-22. Root cause was F4's Windows-only observation generalised to every platform; README, guard and the F4 ledger entry all corrected. NO RELEASE CUT by ruling: pyproject stays 0.4.0, zero occurrences of 0.5.0 in CHANGELOG, no dated heading — 15-6's SC-5 gate has not run. Suite 3188/2 -> 3208 passed / 2 skipped; frontend 80 files / 2305 UNMOVED. DEFERRED AND RELEASE-RELEVANT: `uv run artificial-planeswalker companion` DOES NOT WORK for the plugin install route the README recommends first — uv resolves its project from the CWD and a plugin user has no checkout; homed on 15-5, whose own AC says 'and the companion is launched'. Next: 15-5.
