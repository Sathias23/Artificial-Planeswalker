...
MERGED via PR #90 at 3d700d9 (2026-08-20), Greptile round 1: two P2s, BOTH REAL and both against the story's own new M5 guard (the 15-5 pattern repeating): (1) the transition shorthand is order-free, so the first-token reader missed reversed and implicit-all spellings — fixed by elimination, which promptly surfaced QuantityBadge's `transition: none` flashed rule and forced the none-occupies-the-property-slot correction; (2) toContain(row) matched the rows-as-classes prose note, so a deleted row stayed green — fixed structurally (label + arrow on one line), proven by planting the exact hole (row deleted, label alive in prose -> RED at 2307). Round 2 clean, zero comments. GATE RUN AND CLOSED 2026-08-20 — sc-5-gate-report-2026-08-20.md, ruling line Brad's ("SC-5 is CLOSED; the 0.5.0 release cut is unblocked"). Verdict: accepted, with rulings — open modal != a surface (F1); arrow-key flag RE-ACCEPTED at 206/101; M1 got its tokens.css inventory row, M2/M3 stand on recorded rulings, M5 guard extended (token-usage.test.ts "keys every visual-class transition to an inventory row", 13-entry claims map, planted proof: control 2307, RED on the node, revert clean). Inbox dispositioned by name in deferred-work.md §Dispositions from: the SC-5 gate — J6 DECLINED after five homes, D1 stays unowned by refusal, everything else re-accepted ship-and-adjust. Preceded by Brad's preliminary ruling from the live walk (sc-5-preliminary-ruling-2026-08-20.md).
