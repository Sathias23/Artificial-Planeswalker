...
- **2026-06-28 (post-build smoke-test fix).** Brad's Claude Desktop smoke-test hit `list_decks → "A database error occurred"`. Root cause (reproduced): the optional `user_config.data_dir` was wired into `PLANESWALKER_DATA_DIR=${user_config.data_dir}`; when the field is left blank the host passes the **placeholder literally**, so `paths.py` treated it as a relative path → bogus empty DB → `no such table: decks`. Fix (Brad's call): **dropped the `data_dir` user_config + its env line** from `manifest.json` (env now only `MCP_TRANSPORT`); the server always uses the shared central OS dir. Verified: with the new launch env, `list_decks` returns the real 10 decks. Re-packed. Power-user override still works via host-config `PLANESWALKER_DATA_DIR`/`CARDS_DATABASE_URL`.

- **2026-06-28 (post-build smoke-test fix #2 — timezone-naive timestamps).** After the data-dir fix, deck tools still showed "Tool execution failed" in Claude Desktop while card tools (lookup/search) worked — even a 15 KB search succeeded (ruling out size). Root cause: deck/bug results carry `created_at`/`updated_at`, stored timezone-**naive** in SQLite, so Pydantic emitted `2026-06-25T07:08:56.718473` (no offset) under a `format: "date-time"` schema. Claude Desktop's strict (Ajv-style) RFC 3339 validator **requires a timezone** and rejected the whole result; card tools have no datetimes so they passed. Fix: `field_serializer` on `Deck`, `DeckSummary`/`DeckDetail`, `BugReport` coercing naive → UTC (`…+00:00`). Verified over the real MCP protocol (timestamps pass strict RFC 3339); 256 deck/bug/schema tests pass. NOTE: the server was never broken — it wrote decks fine (`create_deck` actually persisted); only the response was client-rejected. (Files: `src/data/schemas/deck.py`, `src/data/schemas/bug_report.py`; commit `bab4190`. Also added `src/mcp_server/__main__.py` startup stderr diagnostics, `76f6b33`.)
