...
MERGED via PR #89 into `feat/companion-epic-15` at `48f5a79` (2026-08-20). Two commits: `e6ca8ee` the story, `2f2cee2` the Greptile round. All five CI checks green. Mechanises the parity claims where they can go RED: the built tree carries the bundle byte-for-byte and exactly the four product skills (set equality, no `bmad-*`, no Node artefacts); BOTH `build_plugin.py` abort paths fire, pinned by their logged hint; the committed pair is identical and `plugin/` is exactly what a fresh build emits; the mirrored bundle serves through the real SPA mount (200 + `text/javascript`, not a byte comparison standing in for one); and the CI drift step's own preamble is EXECUTED against three repositories. Documents: `docs/plugin-structure.md`, `CONTRIBUTING.md` and `README.md` now say the bundle is a generated artifact in two copies with neither hand-edited, and that the plugin install carries the companion. NO VERSION CUT by ruling — 15-6 is the release gate; both `.mcp.json` files verified unchanged since v0.4.0, not edited. Suite 3208/2 -> 3231 passed / 2 skipped. GREPTILE: 4/5 with two valid P2s, both against this story's own new guards, 5/5 and zero unresolved after one round. (1) The per-subject `ls-files` preamble can only prove a NAMED subject is still tracked, and Vite's content-hashed chunk names cannot be named — so a newly ignored chunk drops out unseen, `git status --porcelain` reporting nothing for ignored paths by construction. All three drift steps (plugin mirror, source SPA, generated types) now ask the complementary question: is git ignoring anything under the tree the build just wrote? (2) The Codex block spent `$PLUGIN_ROOT` without ever assigning it — Codex keeps its own version-keyed cache under `~/.codex`, verified against real installs — so the block now lists the install and assigns the root, bash and PowerShell. The guard written for (2) FIRST PASSED ON THE BUG (document-wide search; the Claude Code block's assignment sits above the Codex block) and is now scoped to each route's own section; caught only by mutating the README back. Closes 15-4's release-relevant deferral. Next: 15-6.
