...
- [x] Firing proofs -- stage the tree; plant (a) the id-mismatch guard removed (every `deck_changed` refetches the active deck), (b) the stale-generation guard removed from the refetch settle (out-of-order response wins); one harness run per plant with `--expect-red`; revert via `git diff --exit-code`; `--expect-green`; paste proof lines here.
  ...
  - Review pass (ten triaged patches, 2026-08-14): +7 tests — 4 in `client.test.ts` (caller-signal merge: abort-reaches-fetch, manual-fallback caller side, manual-fallback timeout side, already-aborted-at-merge), 1 in `deck.test.ts` (`stop()` aborts the in-flight refetch), 2 in `App.test.tsx` (padded-id trim, 503-drop behind an open agent view) — re-certified: `uv run python -m scripts.vitest_probe_harness --expect-total 2150 --expect-green` → `vitest: 75 files / 2150 tests, 0 failed, exit 0`. The two recorded plants stand unchanged; review finding 1's exact revert (merge back to timeout-only) was probed manually and went red on the three caller-side tests before being restored.
...
3. `deck.test.ts`: `stop()`'s abort-the-refetch duty untested → new test pinning the aborted signal and the zero-settle of a late response.
4. `connection.ts`: asymmetric whitespace fold (blank → null but padded-matching id read as a different deck — silent missed refresh) → the fold now trims (blank after trim → null); covered by an App-level padded-id test.
5. `schema.ts` / `socket.ts`: stale cross-citation of `SystemEventKind` rewritten (both spellings of the partition now live in `schema.ts`); `SystemEventKind` deleted outright — its last external consumer was the socket test harness, and a tombstone comment records where the kinds now come from (`SystemEvent['kind']`).
6. `deck.ts`: `driveDeckChanged`'s docstring now records the fourth row honestly — a stopped-yet-settled boot no-ops at `refetch()`'s `!live` gate; unreachable through the production seam, behavior unchanged.
7. `deck.ts`: malformed-200 catch comment corrected — the deck store is untouched, but `seedCardSummaries` runs first and validly-shaped summaries may remain in the shared cache (additive, harmless, deliberately not unwound); the `deck.test.ts` "dropped WHOLE" comment corrected to match.
8. `App.test.tsx`: the four-spellings deck-agnostic test now also pins `formatChecksOf(paths) === 4` — a count where the old per-deck-id rule (1) and the amended per-settled-detail pin (4) genuinely diverge.
9. `App.test.tsx`: the 503-while-a-view-is-open scenario (deleted by the c6-6 amendments) re-covered under the new policy: the drop composes with the overlay — view stays, deck stays behind it, no panel.
10. `App.test.tsx`: the AC 1 recompute test now observes all five derived surfaces MOVING — grid tile, deck-list row, type-group count 1→2, curve bucket 0→1 at MV 2, colour distribution 1→2 green pips — with before-state assertions so each claim is a movement.
...
**Commands:**
- `cd ui && npm run lint && npm run format:check && npm test` -- RUN 2026-08-14, re-run after the review-triage patches: lint clean (eslint + stylelint), prettier clean, `tsc -b` clean, `75 files / 2150 tests, 0 failed` (2123 baseline + 20 story + 7 review-patch tests). Guard suites (store-writes, shell, posture, wire-contract, event-union) pass unchanged — no allow-list or semantics edits anywhere in `ui/tests/`.
